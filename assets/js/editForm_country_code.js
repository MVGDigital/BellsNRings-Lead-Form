...
s_a[105] = "Andaman and Nicobar Islands|Andhra Pradesh|Arunachal Pradesh|Assam|Bihar|Chandigarh|Chhattisgarh|Dadra and Nagar Haveli and Daman and Diu|Delhi|Goa|Gujarat|Haryana|Himachal Pradesh|Jammu and Kashmir|Jharkhand|Karnataka|Kerala|Ladakh|Lakshadweep|Madhya Pradesh|Maharashtra|Manipur|Meghalaya|Mizoram|Nagaland|Odisha|Puducherry|Punjab|Rajasthan|Sikkim|Tamil Nadu|Telangana|Tripura|Uttar Pradesh|Uttarakhand|West Bengal";
...
populateCountries("editCountry", "editState"); */



// Simplify the populateStates function
/* function populateStates(countryElementId, stateElementId) {
    const countryElement = document.getElementById(countryElementId);
    const stateElement = document.getElementById(stateElementId);
    const selectedCountryIndex = countryElement.selectedIndex;
  ...
  // Simplify the populateCountries function
  function populateCountries(countryElementId, stateElementId) {
    const countryElement = document.getElementById(countryElementId);
    countryElement.length = 0;
    countryElement.options[0] = new Option('Select Country', '');
    ...
    country_arr.forEach((country) => {
      countryElement.options[countryElement.length] = new Option(country, country);
    });
  
    // Assign event listener for the states
    countryElement.addEventListener('change', () => {
      populateStates(countryElementId, stateElementId);
    });
  
    // Call populateStates with the selected country on page load
