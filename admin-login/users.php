...
<body>
                  ...
                  <h1>Active Users</h1>
                    ...
                    <div class="tab-content mt_30">
                        <div id="activeUser" class="tab-panel active tab-content">
                            <!-- Active Table -->
                            <div class="addRow_btn mb_30">
								<button id="addUser" class="addRowBtn">
									<img src="../assets1/img/addrowIcon.svg" class="rotateAnime" alt="">Add User
								</button>
                            </div>
                        ...
                        <div id="ready-user" class="tab-panel"></div>
                        <div id="unverified-user" class="tab-panel"></div>
                        <div id="trash-user" class="tab-panel"></div>
                        <div id="settled-user" class="tab-panel"></div>
                        <div id="delete-user" class="tab-panel"></div>
    ...
    button.addEventListener('click', () => {
					...
					columns: [
						 { 
                            ...
                            render: function(data, type, row) {
                                // Render the bnr_id as a clickable link
                                return '<a href="user_view.php?bnr_id=' + data + '">' + data + '</a>';
                            }
                        },
						...
						{ 
							...
							render: function(data, type, row) {
								...
								if (data !== "") {
									// Return HTML for displaying the image ......public_html/storage/app/public/photos.....https://bnrstage.mvgdigital.com/admin-login/storage/app/public/photos5.jpg
									return '<div style="position: relative;">' +
												'<img src="https://bnrstage.mvgdigital.com/storage/app/public/photos/' + data + '" class="ProfImg" alt="Photo" style="width: 100%; height: auto;">' +
												'<button class="downProfImg" data-src="https://bnrstage.mvgdigital.com/storage/app/public/photos/' + data + '" data-bnr-id="' + row.bnr_id +'" style="position: absolute; top: 0; right: 0;"><img src="../assets1/img/prof_down.svg" alt=""></button>' +
											'</div>';
								} else {
						...
						{ 
							...
							render: function(data, type, row) 
							{
								var iconClass = row.registration_status === 'Active' ? 'fa-toggle-on text-success' : 'fa-toggle-off text-danger';
								var nextStatus = row.registration_status === 'Active' ? 'Inactive' : 'Active';
								return '<div><button title="Mark as Settled" class="update_table toggle-status btn btn-link" data-id="' + row.bnr_id + '" data-status="' + nextStatus + '">' +
										   '<i style="font-size:25px;" class="fas ' + iconClass + '"></i>' +
									   '</button>'+
									   '<button class="edit edit_table"><a target="_blank" href="user_edit.php?bnr_id=' + row.bnr_id + '"><img src="../assets1/img/editTable_icon.svg" class="action_icons" alt=""></a></button>' + 
									   ' <button class="view_table"><a target="_blank" href="user_view.php?bnr_id=' + row.bnr_id + '"><img src="../assets1/img/viewTable_icon.svg" class="action_icons" alt=""></a></button>' + 
        ...
        $('#dataTable').on('click', '.update_table', function() {
			var bnrId = $(this).data('id');
			var currentPageUrl = window.location.href; 
			if (confirm('Are you sure you want to mark this profile as settled?', bnrId)) {
				var table;
...
$(document).on('click', '.downProfImg', function() {
    // Retrieve the image source URL from the data-src attribute of the clicked button
    var imgSrc = $(this).data('src');
    // Retrieve the bnr_id from the data-bnr-id attribute of the clicked button
    var bnrId = $(this).data('bnr-id');
    // Check if imgSrc is not empty
    if (imgSrc) {
        var link = document.createElement('a');
        link.href = imgSrc;
        link.download = bnrId + '.png'; // Use bnr_id as the filename
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    }
